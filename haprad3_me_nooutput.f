      program main
      implicit none
      common/var/x,z,q2,pt,un,le,etal,etat,phih,pheta,s,ls,sls,sx,xx,sp,
     & lq,l1,ph0,pl,v1,v2,vp,vm,eh,tmb,   hsf0,   tamin,tamax,rhmax
          real*8 x,z,q2,pt,un,le,etal,etat,phih,pheta,s,ls,sls,sx,xx,sp,
     & lq,l1,ph0,pl,v1,v2,vp,vm,eh,tmb(9),hsf0(9),tamin,tamax,rhmax
      real*8 ebeam,sumsib,sumsigi,sumsigt,sumsibs,sumsigsi,sumsigst,
     & sumsibc,sumsigci,sumsigct,sib,sigi,sigt,del(5,3),urand8
      ...
      character*19 file_name
        data zmin/0.22d0/,zmax/0.77d0/,pt2min/0.015d0/,amhh/0.1395675d0/
     & ,amp/0.938272d0/,m2th/1.1645277295317797d0/
      pi=atan(1d0)*4d0
        amp2=amp**2
      iy=12345_ 8
        nev=-2
...
c        z=zmin+5d-1*dble(iz-1)*(zmax-zmin)
     ...
     -       2*q2*x*(amhh**2*(1 + x - z) + m2th*(-1 + x + z))))
     & /(q2*x*(q2 + 4*amp2*x**2))
        print*,'zis',z
        print*,'pt2up',pt2up
        print*,'pt2max',pt2max
...
c     & sumsigsi/sumsigi,sumsigst/sumsigt,sumsibc/sumsib,sumsigci/sumsigi
c     & ,sumsigct/sumsigt
      write(16,'(22g14.5)') q2, x, z, pt**2,
     & sumsibs/sumsib, sumsigsi/sumsigi, sumsigst/sumsigt,
     & sumsibc/sumsib, sumsigci/sumsigi, sumsigct/sumsigt,
     & sumsib, sumsigi, sumsigt, sumsibs, sumsigsi, sumsigst, sumsibc,
     & sumsigci, sumsigct
      write(*,'(i5,22g14.5)') ipoint, q2, x, z, pt**2,
     & sumsibs/sumsib, sumsigsi/sumsigi, sumsigst/sumsigt,
     & sumsibc/sumsib, sumsigci/sumsigi, sumsigct/sumsigt,
     & sumsib, sumsigi, sumsigt, sumsibs, sumsigsi, sumsigst, sumsibc,
     & sumsigci, sumsigct
     ...
     & lq,l1,ph0,pl,v1,v2,vp,vm,eh,tmb,hsf0,tamin,tamax,rhmax
          real*8 x,z,q2,pt,un,le,etal,etat,phih,pheta,s,ls,sls,sx,xx,sp,
     & lq,l1,ph0,pl,v1,v2,vp,vm,eh,tmb(9),hsf0(9),tamin,tamax,rhmax
          real*8 ebeam,born,borntest,sib,deltavr,vacpol,sig,sigi,sigt,
     & sphi,fspen
          real*8 sh,slsh,xh,slxh,slm,llm,px2
          real*8 URAND8,sum,mu,rh,ta,phik,sigrf,sigrfsphi,sigrfsphiold,
     & sigrfd0,sigex,sigexd0,sigexta,sex,am(3),bm(3),wrk(500),re,rere,
     & rein,reex,ot,otr, phiar(4),tar(6)
          real*8 sum1in,sum2in,sum1ex,sum2ex,ta1,ta2,tam,rhophik,slx,
     & cta1,cta2,xi,rhota
...
c        stop
      ...
      deltavr=2d0*((q2+2d0*ml2)*llm-1d0)*log(rhmax/ml/sqrt(px2))
     & +sh*log((sh+slsh)**2/4d0/ml2/px2)/slsh/2d0
     & +xh*log((xh+slxh)**2/4d0/ml2/px2)/slxh/2d0
     & +sphi(sh,xh,q2+2d0*ml2,ml2,ml2,px2)-2d0+(1.5d0*q2+4d0*ml2)*llm
     & -(q2+2d0*ml2)/slm*(slm**2*llm**2/2d0
     & +2d0*fspen(2d0/(q2/slm+1d0))-pi**2/2d0)
      ta1=-q2/s
      ta2=q2/xx
c      print*, 'tau1 tau2 s xx',ta1,ta2,s,xx
        rein=0d0
        reex=0d0
        if(nev.ge.1)then
      tam=(ta1+ta2)/2d0
       cta1=(s+sls)*(q2*s+2d0*ml2*sx+sls*(sls*tam+sqrt((q2+s*tam)**2
     & +4d0*ml2*(q2+tam*(sx-mp2*tam)))))
     .       /2d0/ml2/(2d0*mp2*q2+sqrt(lq)*sls+s*sx)
       slx=sqrt(xx**2-4d0*ml2*mp2) 
       cta2=(slx+xx)*(sx*xx+sqrt(lq)*slx-2*mp2*q2)/2d0/mp2/(2d0*ml2*sx
     & -q2*xx+slx*(slx*tam
     ...
     & )))*cos(phik)))
      ...
      bm(3)=phiar(iph+1)
       ...
       if(am(1).gt.bm(1))write(*,*)' am(1)<bm(2)'
        ...
        re=re+rere
        print*,'iph,ice,re,rere',iph,ico,re,rere
      enddo  
      enddo
...
c      print*,alfa**3/(256.d0*pi**4*sqly*amp)*ys*sx*barn/1.5435330503633793d0
      ...
      print*,sex,sib,sex/sib
       return     
         ...
         rein=2d0*pi*barn*alpha**3*s*sx**2/64d0/pi**2/mp/pl/ls/sqrt(lq)
     & *rein
      sigi=(alpha/pi*(deltavr+vacpol(q2))+1d0)*sib+rein
         sigt=sigi-alpha**3/(256.d0*pi**4*sqrt(lq)*mp)*sx**2/s*barn/pl*
     & reex
...
c       print*,j,sig-sib,sig/sib
     ...
     & lq,l1,ph0,pl,v1,v2,vp,vm,eh,tmb(9),hsf0(9),tamin,tamax,rhmax
      ...
      data ikey/0/
      pi=atan(1d0)*4d0
      data alpha/0.729735d-2/, barn/0.389379d+6/, mp/0.938272d0/,
     & mn/0.939565d0/, mpi/0.1395675d0/, ml/0.5110d-3/
      !data alpha/.729735d-2/,barn/0.389379d+6/,mp/.938272d0/,mn/.939565d0/,mpi/0.1395675d0/,ml/0.5110d-03/
      !,ml/0.105257d0/
      mp2=mp**2  
      mn2=mn**2  
      ml2=ml**2  
      mpi2=mpi**2  
        ...
        v1=ph0*s/mp-pl*(s*sx+ 2d0*mp2*q2)/mp/sqrt(lq)-2d0*pt*sqrt(l1/lq)
     & *cos(phih)
        v2=ph0*xx/mp-pl*(xx*sx-2d0*mp2*q2)/mp/sqrt(lq)-2d0*pt
     & *sqrt(l1/lq)*cos(phih)
        vp=(v1+v2)/2d0
        vm=(v1-v2)/2d0
        ...
        tmb(8)=-2d0*vp*eh
        tmb(9)=le/2d0/sls*(s*(q2*(z*sx*vp-mpi2*sp)+vm*(s*v2-xx*v1))
     & +2d0*ml2*(4d0*mp2*vm**2+lq*mpi2-z*sx**2*(z*Q2+2d0*vm) ))
       call sfsidis(x,z,q2,pt,un,etal,etat,cos(pheta-phih)
     & ,sin(pheta-phih),hsf0)
c       print*,'sx, lq, q2,phih, tau min and tau max',sx,lq,q2,phih, tamin,tamax 
      end
     ...
     .,futsin2phi2r,rarg
      real*8 x_net(nx),z_net(nz),q2_net(nq2),fuur(nq2,nz,nx),
     & fllr(nq2,nz,nx),futf1tperpr(nq2,nz,nx),futh1r(nq2,nz,nx)
     .,futh1tpr(nq2,nz,nx),fuucos2phir(nq2,nz,nx),fltr(nq2,nz,nx),
     &fulsin2phir(nq2,nz,nx),fltcosphir(nq2,nz,nx),fulsinphir(nq2,nz,nx)
     .,fllcosphir(nq2,nz,nx),fltcos2phir(nq2,nz,nx),
     & fuucosphir(nq2,nz,nx),futsinphisr(nq2,nz,nx),
     & futsin2phi1r(nq2,nz,nx)
     .,futsin2phi2r(nq2,nz,nx),rarg(nx+nz+nq2)
       ...
       read(44,*)x_net(ix),z_net(iz),q2_net(iq2),fuur(iq2,iz,ix),
     & fllr(iq2,iz,ix),futf1tperpr(iq2,iz,ix),futh1r(iq2,iz,ix)
     .,futh1tpr(iq2,iz,ix),fuucos2phir(iq2,iz,ix),fltr(iq2,iz,ix)
     & ,fulsin2phir(iq2,iz,ix),fltcosphir(iq2,iz,ix),
     & fulsinphir(iq2,iz,ix),fllcosphir(iq2,iz,ix),
     & fltcos2phir(iq2,iz,ix),fuucosphir(iq2,iz,ix),
     & futsinphisr(iq2,iz,ix),futsin2phi1r(iq2,iz,ix)
     ...
     .f45in,f46rn,f46in,f56rn,f56in,rargex
      ...
      real*8 q2net(nq2),wnet(nw),tnet(nt),f11n(nt,nw,nq2),
     & f22n(nt,nw,nq2),f33n(nt,nw,nq2),f44n(nt,nw,nq2),f55n(nt,nw,nq2)
     & ,f66n(nt,nw,nq2),f12rn(nt,nw,nq2),f12in(nt,nw,nq2),
     & f13rn(nt,nw,nq2),f13in(nt,nw,nq2),f14rn(nt,nw,nq2),
     .f14in(nt,nw,nq2),f15rn(nt,nw,nq2),f15in(nt,nw,nq2),
     & f16rn(nt,nw,nq2),f16in(nt,nw,nq2),
     .f23rn(nt,nw,nq2),f23in(nt,nw,nq2),f24rn(nt,nw,nq2),
     & f24in(nt,nw,nq2),f25rn(nt,nw,nq2),
     .f25in(nt,nw,nq2),f26rn(nt,nw,nq2),f26in(nt,nw,nq2),
     & f34rn(nt,nw,nq2),f34in(nt,nw,nq2),
     .f35rn(nt,nw,nq2),f35in(nt,nw,nq2),f36rn(nt,nw,nq2),
     & f36in(nt,nw,nq2),f45rn(nt,nw,nq2),
     .f45in(nt,nw,nq2),f46rn(nt,nw,nq2),f46in(nt,nw,nq2),
     & f56rn(nt,nw,nq2),f56in(nt,nw,nq2),rargex(nq2+nw+nt)
      ...
      real*8 w,t,q2       
       ...
       do iq2=1,nq2
        do iw=1,nw
          do it=1,nt
         read(44,*)q2net(iq2),wnet(iw),tnet(it),a1r,a1i,a2r,a2i,a3r,a3i,
     ...
     & *Sqrt((W**2-mp2-q2)**2/(4.*W**2) + q2))
        f1r= a1r+(2d0*a6r*q2-(a3r-a4r)*(q2+t-mpi2))/
     & 2d0/(w-mp)+a4r*(w-mp)
        f1i= a1i+(2d0*a6i*q2-(a3i-a4i)*(q2+t-mpi2))/
     & 2d0/(w-mp)+a4i*(w-mp)
        f2r=-a1r+(2d0*a6r*q2-(a3r-a4r)*(q2+t-mpi2))/
     & 2d0/(w+mp)+a4r*(w+mp)
        f2i=-a1i+(2d0*a6i*q2-(a3i-a4i)*(q2+t-mpi2))/
     & 2d0/(w+mp)+a4i*(w+mp)
...
c      PRINT *, 'The f12r is ', f12rn
      ...
      real*8 function borntest(x,z,q2,pt,ebeam,un,etal,le,etat,phih
     & ,pheta)
      implicit none
      ...
      real*8 x,z,q2
      common/fstrf/fuu,fll,futf1tperp,futh1,futh1tp,fuucos2phi,flt,
     & fulsin2phi,
     . fltcosphi, fulsinphi, fllcosphi, fltcos2phi, fuucosphi,
     & futsinphis,futsin2phi
     ...
     & futsinphis,futsin2phi
       ...
       borntest=2d0*pi*barn*alpha**2*y**2/(2d0*x*y*q2*(1d0-eps))
     & *(1d0+ga**2/2d0/x)*(un*(fuu+sqrt(2d0*eps*(1d0+eps))*cos(phih)
     & *fuucosphi+eps*cos(2d0*phih)*fuucos2phi)
     . +etal*sqrt(2d0*eps*(1d0+eps))*sin(phih)*fulsinphi
     & +etal*eps*sin(2d0*phih)*fulsin2phi+etal*le*(sqrt(1-eps**2)*fll
     . +sqrt(2d0*eps*(1d0-eps))*cos(phih)*fllcosphi)
     & +etat*(sin(phih-pheta)*futf1tperp+eps*sin(phih+pheta)*futh1
     . +eps*sin(3d0*phih-pheta)*futh1tp+sqrt(2d0*eps*(1d0+eps))
     & *(sin(pheta)*futsinphis+sin(2d0*phih-pheta)*futsin2phi)
     . +le*(sqrt(1-eps**2)*cos(phih-pheta)*flt+sqrt(2d0*eps*(1d0-eps))
     & *(cos(pheta)*fltcosphi+cos(2d0*phih-pheta)*fltcos2phi))))
...
c
	 ...
	 allmi=log((sqlmi+t)/(sqlmi-t))/sqlmi
       suml=suml+2.d0*(t+a2)*allmi/3.d0-10.d0/9.d0+4.d0*a2*
     & (1.d0-a2*allmi)/3.d0/t
       enddo 
      if(t.lt.1.d0)then
	aaa = -1.345d-9
	bbb = -2.302d-3
	...
	ccc =  9.9992d-1
      ...
      sigrfd0=sigrf(arg(1),arg(2),arg(3))
        print*,'arg(1),arg(2),arg(3),sigrfd0',arg(1),arg(2),arg(3),sigrfd0
      return
      end
...
c      subroutine simpux(a,b,np,ep,func,res)
     ...
     & lq,l1,ph0,pl,v1,v2,vp,vm,eh,tmb(9),hsf0(9),tamin,tamax,rhmax
      ...
      sigrfsphiold=0d0
       do i=1,5
      call simptx(log(x+tar(i))+ep,log(x+tar(i+1))-ep,100,1d-3,
     & sigrfstaold,re)
...
c            print*,'2',ptr
         ptr=sqrt((z**2*sx**2-(2d0*mp2*mu*r+2d0*mp*pl*sqrt(lq)-z*sx*r)**
     & 2/lqr)/4d0/mp2-mpi2)
            if(isnan(ptr))then
                sigrf=0d0
                return
...
c            print*,'2',ptr
c                stop
            if(isnan(ptr))print*,ph0**2-(z*sx*(sx-r)-2d0*mp2*(2d0*vm-mu*
     & r))**2/4d0/mp2/((sx-r)**2+4d0*mp2*q2r)-mpi2
            if(etal**2+etat**2.gt.5d-1)then
********** scalar product vector decomposition
 
           etaq=-etal*sqrt(lq)/2d0/mp
           etak=-r*(etal*(sx-2d0*mp2*ta)+2d0*etat*mp2*sqrt((tamax-ta)*
     & (ta-tamin))*cos(phik-pheta))/2d0/mp/sqrt(lq)
           etak1=-(etal*(s*sx+2d0*mp2*q2)+2d0*mp*etat*sqrt(l1)
     & *cos(pheta))/2d0/mp/sqrt(lq)
           etaph=-etal*pl-etat*pt*cos(phih-pheta)
***********************************8
 
         etalr=(etal*(lq+r*(2d0*ta*mp2-sx))-2d0*etat*mp2*r*cos(pheta-
     & phik)*sqrt((ta-tamin)*(tamax-ta)))/sqrt(lq*lqr)
c         etalr=2d0*mp*(etak-etaq)/sqrt(lqr)
          etatr=sqrt(etat**2+4d0*mp2*r*sqrt((tamax-ta)*(ta-tamin))*(mp2*
     & r*sqrt((tamax-ta)*(ta-tamin))*(etal**2-etat**2*cos(phik-pheta)**2
     & )+etal*etat*cos(phik-pheta)*(lq+r*(2d0*ta*mp2-sx)))/lq/lqr)  
c            print*,'4',etalr,etatr

        cdph=(-etaph+((etak-etaq)*(r*sx*z-2d0*sqrt(lq)*mp*pl-2*mp2*mu*r)
     & )/((r-sx)**2+4*mp2*(q2+r*ta)))/etatr/ptr
...
cx        cdph=(-etaph+(etalr*(r*sx*z-2d0*sqrt(lq)*mp*pl-2d0*mp2*mu*r))/2d0/mp/sqrt((r-sx)**2+4*mp2*(q2+r*ta)))/etatr/ptr
        sdph=-(-(((sx*(2*s*(Vm+q2*z)-sx*(V1+q2*z)))+r**2*(mu*s-V1+
     & sx*z*z1)+r*(2*sx*V1 + s*(2*sx*ta*z-mu*sx-2*Vm)+sx*z*(q2-sx*z1))
     & -2*mp2*(q2*(mu*r+2*Vp)+r*(2*ta*V1+mu*r*z1-2*Vm*z1)))*(-etat*
     & (2*ml2*(sx*(sx-r)+2*mp2*(2*q2+r*ta))+2*mp2*q2*(q2+r*z1)-s*(r*s*ta
     & +q2*(r+2*xx)-r*sx*z1))*Sin(pheta)+(2*mp*r*(etaq*(4*ml2*mp2-s**2)+
     & etak1*(2*mp2*q2+s*sx))*Sqrt((tamax-ta)*(ta-tamin))*Sin(phik))
     & /Sqrt(lq)))+2*(etak1*((sx-r)**2+4*mp2*(q2+r*ta))+(etak-etaq)*
     & (s*(sx-r)+2*mp2*(q2+r*z1)))*
     &       (pt*(2*ml2*(sx*(sx-r)+2*mp2*(2*q2+r*ta))+2*mp2*q2*(q2+r*z1)
     & -s*(r*s*ta+q2*(r+2*xx)-r*sx*z1))*Sin(phih) + 
     &       (mp*r*Sqrt((tamax-ta)*(ta-tamin))*(s*(V1*xx+q2*sx*z-s*V2)
     & -2*mp2*q2*V1+2*ml2*(sx**2*z-4*mp2*Vm))*Sin(phik))/Sqrt(lq)))/
     &      (4.*etatr*ptr*((sx-r)**2+4*mp2*(q2+r*ta))**1.5*sqrt(l1)*
     & ((q2*s*(r+xx)-mp2*(q2+r*z1)**2+r*s*(s*ta+(r-sx)*z1))/
...
c                print*,'c2',cdph,sdph,cdph**2+sdph**2
        ...
        call sfsidis(xr,zr,q2r,ptr,un,etal,etat,cos(pheta-phih)
     & ,sin(pheta-phih),hsf)
        endif
        sum=0d0
...
c            print*,'vm,mu,Rex,ta,phik,q2r,w2r',vm,mu,r,ta,phik,q2r,w2r
            if(etal**2+etat**2.gt.5d-1)then

         lqr=(sx-r)**2+4d0*mp2*q2r   
         etalr=(etal*(lq+r*(2d0*ta*mp2-sx))-2d0*etat*mp2*r*cos(pheta-phi
     & k)*sqrt((ta-tamin)*(tamax-ta)))/sqrt(lq*lqr)
          etatr=sqrt(etat**2+4d0*mp2*r*sqrt((tamax-ta)*(ta-tamin))*(mp2*
     & r*sqrt((tamax-ta)*(ta-tamin))*(etal**2-etat**2*cos(phik-pheta)**2
     & )+etal*etat*cos(phik-pheta)*(lq+r*(2d0*ta*mp2-sx)))/lq/lqr)
     ...
     & )/((r-sx)**2+4*mp2*(q2+r*ta)))/etatr/ptr
        sdph=-(-(((sx*(2*s*(Vm+q2*z)-sx*(V1+q2*z)))+r**2*(mu*s-V1
     & +sx*z*z1)+r*(2*sx*V1 + s*(2*sx*ta*z-mu*sx-2*Vm)+sx*z*(q2-sx*z1))
     & -2*mp2*(q2*(mu*r+2*Vp)+r*(2*ta*V1+mu*r*z1-2*Vm*z1)))*
     & (-etat*(2*ml2*(sx*(sx-r)+2*mp2*(2*q2+r*ta))+2*mp2*q2*(q2+r*z1)
     & -s*(r*s*ta+q2*(r+2*xx)-r*sx*z1))*Sin(pheta)
     & +(2*mp*r*(etaq*(4*ml2*mp2-s**2)
     & +etak1*(2*mp2*q2+s*sx))*Sqrt((tamax-ta)*(ta-tamin))
     & *Sin(phik))/Sqrt(lq))) 
     & +2*(etak1*((sx-r)**2+4*mp2*(q2+r*ta))+(etak-etaq)*(s*(sx-r)
     & +2*mp2*(q2+r*z1)))*(pt*(2*ml2*(sx*(sx-r)+2*mp2*(2*q2+r*ta))
     & +2*mp2*q2*(q2+r*z1)-s*(r*s*ta+q2*(r+2*xx)-r*sx*z1))*Sin(phih)+
     & (mp*r*Sqrt((tamax-ta)*(ta-tamin))*(s*(V1*xx+q2*sx*z-s*V2)
     & -2*mp2*q2*V1+2*ml2*(sx**2*z-4*mp2*Vm))*Sin(phik))/Sqrt(lq)))
     & /(4.*etatr*ptr*((sx-r)**2+4*mp2*(q2+r*ta))**1.5*sqrt(l1)*((q2*s*
     & (r+xx)-mp2*(q2+r*z1)**2+r*s*(s*ta+(r-sx)*z1))/((r-sx)**2+4*mp2*
     & (q2+r*ta))-ml2))
...
c               print*,'c1',cdph,sdph,cdph**2+sdph**2
        ...
        call sfexclus(q2r,w2r,mpi2-q2-2d0*vm+(mu-ta)*r,un,etalr,etatr,
     & cdph,sdph,hsfex)
            else
        call sfexclus(q2r,w2r,mpi2-q2-2d0*vm+(mu-ta)*r,un,etal,etat,
     & cos(pheta-phih),sin(pheta-phih),hsfex)
            endif
            else
        call sfexclus(q2r,w2r,mpi2-q2-2d0*vm+(mu-ta)*r,un,etal,etat,
     & cos(pheta-phih),sin(pheta-phih),hsfex)
     ...
     & f13rn(nt,nw,nq2),f13in(nt,nw,nq2),f14rn(nt,nw,nq2),
     & f14in(nt,nw,nq2),f15rn(nt,nw,nq2),f15in(nt,nw,nq2),
     & f16rn(nt,nw,nq2),f16in(nt,nw,nq2),f23rn(nt,nw,nq2),
     & f23in(nt,nw,nq2),f24rn(nt,nw,nq2),f24in(nt,nw,nq2),
     & f25rn(nt,nw,nq2),f25in(nt,nw,nq2),f26rn(nt,nw,nq2),
     & f26in(nt,nw,nq2),f34rn(nt,nw,nq2),f34in(nt,nw,nq2),
     & f35rn(nt,nw,nq2),f35in(nt,nw,nq2),f36rn(nt,nw,nq2),
     & f36in(nt,nw,nq2),f45rn(nt,nw,nq2),f45in(nt,nw,nq2),
     & f46rn(nt,nw,nq2),f46in(nt,nw,nq2),f56rn(nt,nw,nq2),
     & f56in(nt,nw,nq2),rargex(nq2+nw+nt),
     & a2,a30,a31,a3,w2arg,wcor,q2cor,q2arg
...
c      print*,'chect ex thcm,w2arg,q2arg',180d0/pi*thcm,w2arg,q2arg
        arg(1)=180d0/pi*thcm
...
c        print*,'check sx,lq,zz,pt',sx,lq,zz,pt
     ...
     & +4*w*f23r)+2*r5*(mp2-w2)*f34r)+2*etat*sdph*(r5*(f14i*r1*(mp-W)**2
     & -f23i*r2*(mp+w)**2)+lq*(r4*f24i-r3*f13i-4*w*f12i+2*pt**2*w*f34i)*
     & (w2-mp2))/sqrt(lq)/pt)/2./w2    
        w01r=pt*sqrt(q2)*un*(r1*(mp-W)*(r4*f46r+2*w*f16r)-r2*(mp+w)*(r3*
     & f35r+2*w*f25r)+r5*(f45r*(w-mp)+ (mp + W)*f36r))/sqrt(lq)/w + 
     & (etat*sqrt(q2)*sdph*(r5*(f16i*r1*(mp-w)+f25i*r2*(mp+w))
     & -lq*(f15i*r3*(mp-w)+f26i*r4*(mp+w)+2*pt**2*W*(f45i*(mp-W)
     & +f36i*(mp+w)))))/r1/r2/w
        w01i=pt*Sqrt(Q2)*un*(r1*(w-mp)*(r4*f46i+2*w*f16i)+r2*(mp+w)*
     & (f35i*r3+2*f25i*w)+r5*(f45i*(mp-w)-f36i*(mp+w)))/sqrt(lq)/w 
     & +etat*sqrt(q2)*sdph*(r5*(f16r*r1*(mp-w)+f25r*r2*(mp+w))
     & -lq*(f15r*r3*(mp-w)+f26r*r4*(mp+w)+2*pt**2*w*(f45r*(mp-w)
     & +f36r*(mp+w))))/r1/r2/W
        w02r=2*etal*pt*Sqrt(Q2)*(f25i*r2*(mp+w)-f16i*r1*(mp-w))/Sqrt(lq)
     & +cdph*etat*Sqrt(Q2)*((f16i*r1*r5-f15i*lq*r3)*(w-mp)
     & +r2*(f26i*r1*r4-f25i*r5)*(mp+w))/r1/r2/w    
        w02i=2*etal*pt*Sqrt(Q2)*(f25r*r2*(mp+w)-f16r*r1*(mp-w))/Sqrt(lq)
     &  +(cdph*etat*Sqrt(Q2)*((f16r*r1*r5-f15r*lq*r3)*(w-mp)
     & +r2*(f26r*r1*r4-f25r*r5)*(mp+w)))/r1/r2/w
        w12r=-etal*pt**2*(f14i*r1*(mp-w)**2+f23i*r2*(mp+w)**2)/w + 
     & cdph*etat*pt*(r2*(mp+w)*(f23i*r5*(mp+w)+r1*(mp-w)*
     ...
     &  (cdph*etat*pt*(-f14r*r1*r5*(mp-W)**2+r2*(f23r*r5*(mp+W)**2+f24r
     & *r1*r4*(mp2-w2))+f13r*lq*r3*(w2-mp2)))/ (2.*Sqrt(lq)*W**2)
        hsfex(1)=w22/8d0/pi/alpha
        hsfex(2)=(4d0*(q2*w00p+sqrt(q2/lq)*r0/pt*w01r)+r0**2/lq*w11p)/lq
     & /8d0/pi/alpha
        hsfex(3)=w11p/8d0/pi/alpha 
        hsfex(4)=-(r0/lq*w11p+2d0*sqrt(q2/lq)*w01r/pt)/8d0/pi/alpha
        hsfex(5)=Sqrt(q2/lq)*w01i/pt/4d0/pi/alpha 
        hsfex(6)=-(2d0*sqrt(q2)*w02r+r0*w12r/pt/sqrt(lq))/pt/lq/4d0/pi/
     & alpha
        hsfex(7)=(2d0*sqrt(q2)*w02i+r0*w12i/pt/sqrt(lq))/pt/lq/4d0/pi/
     & alpha
        hsfex(8)=w12r/sqrt(lq)/pt**2/4d0/pi/alpha
        hsfex(9)=-w12i/sqrt(lq)/pt**2/4d0/pi/alpha
            
c        print*,'r0,r1,r2,r3,r3,r5',r0,r1,r2,r3,r4,r5
c        print*,'it,iw,iq2,q2,w,t',it,iw,iq2,q2,w,t
c        print*,'f1r,f2r',f1r,f2r
c        print*,'wcor,q2cor'
c        print'(13(g12.5))',wcor,q2cor
c        print*,'f11,f22,f33,f44,f55,f66'
c        print'(13(g12.5))',f11,f22,f33,f44,f55,f66
c        print*,'f11,f12r,f12i'
c        print'(13(g12.5))',f11,f12r,f12i,f13r,f13i,f14r,f14i,f15r,f15i
c     & ,f16r,f16i,f22,f23r,f23i,f24r,f24i,f25r,f25i,f26r,f26i,f33,f34r
c     & ,f34i,f35r,f35i,f36r,f36i,f44,f45r,f45i,f46r,f46i,f55,f56r,f56i
c     & ,f66
c        print*,'un',un
c        print*,'w00p,w22,w11p,w01r,w01i,w02r,w02i,w12r,w12i'
c        print'(13(g12.5))',w00p,w22,w11p,w01r,w01i,w02r,w02i,w12r,w12i
c        print*,'Hsfex(1,2,3,4,5,6,7,8,9)'
c        print'(13(g12.5))',hsfex(1),hsfex(2),hsfex(3),hsfex(4),hsfex(5)
c     &   ,hsfex(6),hsfex(7),hsfex(8),hsfex(9)
c     check each exclusive structure functions
     ...
     &   (mp+W)**2+f24r
     & *r1*r4*(mp2-w2))+f13r*lq*r3*(w2-mp2)))/ (2.*Sqrt(lq)*W**2)
c        print*,'H00pH22_000,H11mH22opt2_000,H22_000,H01r_000,H01i_000,
c     &  H00pH22_010,H11mH22opt2_010,H22_010,H01r_010,H01i_010,H02r_100,
c     &  H02i_100,H12r_100,H12i_100,H02r_001,H02i_001,H12r_001,H12i_001'
c        print*,H00pH22_000,H11mH22opt2_000,H22_000,H01r_000,H01i_000,
c     &  H00pH22_010,H11mH22opt2_010,H22_010,H01r_010,H01i_010,H02r_100,
c     &  H02i_100,H12r_100,H12i_100,H02r_001,H02i_001,H12r_001,H12i_001
...
c      print*,'fir',fir,ml2*bis,q2+2d0*ml2,bid  
        ...
        tmr(2,2)=(sp*sx*bip+2d0*ml2*sp*bir+2d0*(sx-2d0*mp2*ta)*fir-bid
     & *sp**2*ta)/2d0
        tmr(2,3)=((4d0*ml2+ta*(2d0*ta*mp2-sx))*bid-sp*bip+4d0*mp2)/2d0
        tmr(2,4)=0d0
        tmr(3,1)=4d0*fir*(v1*v2-mpi2*q2)/2d0
        tmr(3,2)=2d0*((mu*vm-ta*mpi2)*fir+vp*(mu*ml2*bir+bip*vm-
     & ta*vp*bid))
        tmr(3,3)=(2d0*mu**2*ml2 + ta*(mpi2*ta - mu*vm))*bid
     & -mu*vp*bip+2d0*mpi2
        tmr(3,4)=0d0
        tmr(4,1)=4d0*fir*(s*v2+xx*v1-z*q2*sx)/2d0
        tmr(4,2)=(s*v1-v2*xx)*bip+ml2*(mu*sp+2d0*vp)*bir-2d0*ta*sp*vp
     & *bid+((mu-2d0*ta*z)*sx+2d0*vm)*fir
        tmr(4,3)=((8d0*mu*ml2+ta*((2d0*ta*z-mu)*sx-2d0*vm))*bid
     & -(mu*sp+2d0*vp)*bip + 4d0*sx*z)/2d0
        tmr(4,4)=0d0
...
c        stop
        ...
        tmr(5,2)=le*s/l1/sls*(eh*(2d0*(sx*(q2+4d0*ml2)+2d0*ta*(s*xx-2d0
     & *mp2*(2d0*ml2+q2)))*fir +q2*(sp*(sx*bip+2d0*ml2*bir)
     & -ta*(4*s*xx+sx**2)*bid))+2d0*ekr*(ml2*(sx*(s*v2-v1*xx-q2*sp*z)
     & +4d0*mp2*q2*vp)*bir+((q2+4d0*ml2)*(4d0*mp2*vm-sx**2*z)
     & +sp*(s*v2-v1*xx))*fir))
cccccccccccccccc (B2) contribution 
     & +2d0*le*ml2/l1/sls*(eh*(2d0*(2d0*ml2*lq+(q2+s*ta)
     & *(2d0*mp2*q2+s*sx))*bi21-sx*lq*bip+(2d0*q2*xx*sx+ta*sx*(2d0*s**2
     & -sp**2)+4d0*mp2*q2*(ta*s-q2)-4d0*ml2*lq)*bid)+2d0*ekr*(sx*
     & (z*sp*q2+v1*xx-s*v2)-4d0*mp2*q2*vp)*(bid*xx-bi21*s))  
...
c     .           +2d0*bi21*le*s/l1/sls*(ekr*(2d0*(mu*q2+ta*v1)*(s*xx-mp2*q2)+(q2+s*ta)*(z*q2*sx-s*v2-v1*xx))-eh*(q2+s*ta)**2)
        tmr(5,3)=(le*s*(eh*(2*lq - bip*q2*sp + bid*ta*((4*ml2 + q2)*
     & (-sx + 2*mp2*ta) - 2*ta*(-(mp2*q2) + s*xx))) + ekr*(bip*(-(lq*vp)
     & + sp*sx*(vm + q2*z)) + bid*ta*(sp*(-(s*v2) + v1*xx) - 
     & (4*ml2 + q2)*(4*mp2*vm - sx**2*z)))))/ (l1*sls) 
cccccccccccccccc (B2) contribution 
     & +2d0*le*ml2/l1/sls*(eh*(2d0*((q2+2d0*ml2)*(2d0*ta*mp2+xx)
     & -(ta*xx+2d0*ml2)*s)*bi21-lq*bip 
     & +(4d0*ml2*(sx-2d0*ta*mp2)+2d0*q2*s+ta*(s**2+xx**2))*bid) 
     & +2d0*ekr*((2d0*ml2*(z*sx**2-4d0*mp2*vm)
     & +2d0*mp2*q2*v2+xx*(v1*xx-s*v2-q2*sx*z))*bi21
     & +(2d0*ml2*(4d0*mp2*vm-z*sx**2)
     & +2d0*mp2*q2*v1+s*(s*v2-v1*xx-q2*sx*z))*bid))
        tmr(5,4)=0d0
        tmr(6,1)=-4d0*fir*sp*eh
        tmr(6,2)=(eh*((4d0*mp2*q2*(q2+4d0*ml2)-sx**2*(q2-4d0*ml2)-
     & 8d0*q2*s*xx)*(sx*bip+2d0*ml2*bir-ta*sp*bid) 
     ...
     & +((q2+4d0*ml2)*(z*sx**2-4d0*mp2*vm)+sp*(v1*xx-s*v2))*fir))/2d0/l1
        tmr(6,3)=(2d0*eh*((2d0*q2*(s*xx-2d0*mp2*q2)
     & -ta*sx*(sx**2+3d0*s*xx-4d0*ml2*mp2)-(q2+2d0*ml2)*sx**2)*bip+ml2*
     & (2d0*ta*(2d0*mp2*(q2+2d0*ml2)-s*xx)-sx*(q2+4d0*ml2))*bir
     & -q2*sp*fir+sp*(ta**2*(sx**2+2d0*s*xx-2d0*mp2*(q2+4d0*ml2))
     & +2d0*ta*sx*(q2+2*ml2)-4d0*ml2*q2)*bid+sp*sx**2)
     & +ekr*(((q2+4d0*ml2)*(z*sx**2-4d0*mp2*vm)+sp*(v1*xx-s*v2))*
     & (2*ml2*bir+sx*bip)+2d0*ml2*(sx*(z*q2*sp-s*v2+v1*xx)
     & -4d0*mp2*q2*vp)*bis +(4d0*ta*(mp2*q2*(4d0*s*vm+sx*(v2-vm))
     & +2d0*s*xx*(s*v2-v1*xx)+2d0*ml2*sp*(4d0*mp2*vm - z*sx**2))
     & +sx*(3d0*ta*sx+2d0*(q2-2d0*ml2))*(s*v2-v1*xx-z*q2*sp)
     &+8d0*mp2*q2*(q2-2d0*ml2)*vp)*bid))/2d0/l1
        tmr(6,4)=(eh*(((q2+4d0*ml2)*sx+2d0*ta*(s*xx-2d0*mp2*(q2+2d0*ml2)
     & ))*bip+sp*(ta*q2*bid-2*sx)) +ekr*(((q2+4d0*ml2)*
     & (4d0*mp2*vm-sx**2*z)+sp*(s*v2-v1*xx))*bip 
     & +bid*ta*(4*mp2*q2*vp+sx*(s*v2-v1*xx-z*q2*sp))))/2d0/l1
        tmr(7,1)=fir*le*s/sls*(lq*vp-sp*sx*(z*q2+vm)) 
        tmr(7,2)=le*s/2d0/sls*(q2*(4d0*mp2*vm-sx**2*z)*bip
     & +ml2*(mu*lq-2d0*sx*(z*q2+vm))*bir+(2d0*(4d0*ta*mp2-sx)*vp
     & +(mu-2d0*ta*z)*sp*sx-2d0*s*v2+2d0*v1*xx)*fir+ta*(q2*
     & (z*sp*sx-4d0*mp2*vp)+sx*(v1*xx-s*v2))*bid)
cccccccccccccccc (B2) contribution 
     & +le*ml2/sls*((4*mp2*(ta*s*vm-q2*vp)-sx**2*(ta*z*s+z*q2+v1)
     & +mu*lq*s)*bi21+bid*(4*mp2*(q2*vp-ta*vm*xx)+sx**2*
     & (ta*z*xx+v2-z*q2)-mu*lq*xx))
        tmr(7,3)=le*s/4d0/sls*((sx*(4d0*z*q2+2d0*vm-mu*sx)-8d0*mp2*mu*q2
     & )*bip+2d0*ml2*(4d0*mu*ta*mp2+2d0*vm-sx*(mu+2d0*ta*z))*bir 
     & +2d0*(2d0*vp-mu*sp)*fir+ta*(4d0*(sx-2d0*ta*mp2)*vp
     & +sp*(sx*(2d0*ta*z-mu)-2d0*vm))*bid)
cccccccccccccccc (B2) contribution 
